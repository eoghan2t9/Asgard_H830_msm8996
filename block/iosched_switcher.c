...
static void change_elevator(struct req_queue_data *r, bool use_block)
{
	struct request_queue *q = r->queue;

	if (r->using_block == use_block)
		return;

	r->using_block = use_block;

	if (use_block) {
		strcpy(r->prev_e, q->elevator->type->elevator_name);
		elevator_change(q, BLOCK_IOSCHED);
	} else {
		elevator_change(q, r->prev_e);
	}
}

static void change_all_elevators(struct list_head *head, bool use_block)
{
	struct req_queue_data *r;

	list_for_each_entry(r, head, list)
		change_elevator(r, use_block);
}
		...
		unsigned long action, void *data)
{
	...
	case FB_BLANK_UNBLANK:
		/*
		 * Switch back from screen off iosched to the original iosched after a delay
	...
	default:
		/*
		 * Switch to defined iosched when the screen turns off. Purposely block
